...
export function activate(context: vscode.ExtensionContext) {
  context.subscriptions.concat([
    ...
    vscode.commands.registerTextEditorCommand(
      "regExpSaver.findInFile",
      findInFile
    ),
    vscode.commands.registerTextEditorCommand(
      "regExpSaver.findInSelection",
      findInSelection
    ),
    vscode.commands.registerTextEditorCommand(
      "regExpSaver.replaceInFile",
      replaceInFile
    ),
    vscode.commands.registerTextEditorCommand(
      "regExpSaver.replaceInSelection",
      replaceInSelection
    ),
  ]);
}
...
async function saveNew() {
  ...
  const replacePattern = await vscode.window.showInputBox({
    prompt:
      "(Optional) Enter your replace pattern, or blank to delete all matches",
    ignoreFocusOut: true,
  });
  ...
  const globalOrWorkspace = await vscode.window.showQuickPick(
    ...
    {
      title: "Save RegExp globally or just for this workspace?",
      ignoreFocusOut: true,
    }
  );
...
async function pickSavedItem(): Promise<SavedItem | undefined> {
  ...
  const savedItemsForWorkspace: any[] = configuration.get(
    "regExpSaver.savedForWorkspace",
    []
  );
