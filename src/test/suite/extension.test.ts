...
describe("Extension Test Suite", function () {
  ...
  before(async function () {
    ...
    previousSavedItems = vscode.workspace
      .getConfiguration()
      .get("regExpSaver.saved");
  });
  ...
  beforeEach(async function () {
    ...
    await vscode.workspace
      .getConfiguration()
      .update("regExpSaver.saved", undefined, true);
    await vscode.workspace
      .getConfiguration()
      .update("regExpSaver.savedForWorkspace", undefined, false);
  });
  ...
  after(async function () {
    ...
    await vscode.workspace
      .getConfiguration()
      .update("regExpSaver.saved", previousSavedItems, true);
  });
  ...
  it("can save new RegExp globally", async function () {
    ...
    const savedForWorkspace = vscode.workspace
      .getConfiguration()
      .get("regExpSaver.savedForWorkspace");
  ...
  it("can replace in file", async function () {
    ...
    await vscode.workspace
      .getConfiguration()
      .update("regExpSaver.saved", [savedRegExp], true);
    ...
    await textEditor.edit((builder) =>
      builder.replace(document.positionAt(0), text)
    );
